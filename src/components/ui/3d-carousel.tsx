...
const Carousel = memo(
  ({
    ...
    return (
      <div
        className="flex h-full items-center justify-center bg-amber-50/20"
        style={{
          perspective: "1000px",
      ...
      >
        ...
        >
          {cards.map((imgUrl, i) => (
            <motion.div
              key={`key-${imgUrl}-${i}`}
              className="absolute flex h-full origin-center items-center justify-center rounded-xl bg-amber-50/10 p-2"
              style={{
                width: `${faceWidth}px`,
                transform: `rotateY(${
                  i * (360 / faceCount)
...
function ThreeDPhotoCarousel() {
  ...
  const cards = useMemo(
    () => keywords.map((keyword) => `https://source.unsplash.com/400x400/?${keyword}`),
    []
  )
